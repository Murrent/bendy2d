...
impl ParticleLink {
    pub fn solve(&mut self, particles: &mut Vec<Particle>) {
        ...
        let diff = (dist - self.link.target_distance) / (particle_a.inv_mass + particle_b.inv_mass);
        particle_a.pos -= particle_a.inv_mass * dist_vec.normalize() * diff * 0.5;
        particle_b.pos += particle_b.inv_mass * dist_vec.normalize() * diff * 0.5;
    }
}
